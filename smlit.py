...
def main():
    page = st.sidebar.radio("Select Page", ["Motif Summary", "Motif Analysis", "Promoter Prediction"])
...
def page1():
    st.title("Deterministic Motif Finding App")
...
def page2():
    st.title("Promoter Sequence Prediction")
    ...
    if st.button("Classify Sequence"):
        if user_input:
            ...
            if predicted_class == 1:
                st.success("Promoter detected!")
            else:
                st.info("No promoter mark found.")
